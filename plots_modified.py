...
plt.rcParams["font.serif"] = ["DejaVu Serif"]  # Font sempre presente in matplotlib
...
def plot_predictions(experiment_path: str,
    ...
    profiles_hat_to_save = denormalise(predictions, scaling_factors_min, scaling_factors_max)
    profiles_targets_to_save = denormalise(targets, scaling_factors_min, scaling_factors_max)

    # Converti in numpy (nel caso siano torch.Tensor)
    profiles_hat_np = profiles_hat_to_save.detach().cpu().numpy()
    profiles_targets_np = profiles_targets_to_save.detach().cpu().numpy()

    # Salva in formato MATLAB (.mat)
    savemat(save_dir / "predictions_vs_targets.mat", {
        "profiles_hat": profiles_hat_np,
        "profiles_targets": profiles_targets_np
    })
